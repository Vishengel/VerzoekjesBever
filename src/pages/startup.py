    ...
    with ui.column().classes("w-full max-w-lg mx-auto mt-16 gap-6"):
        ...
        if svc.has_session:
            with ui.card().classes("w-full bg-green-900/30 border border-green-700"):
                ui.label("Previous session found").classes("text-lg font-semibold text-green-400")
                ui.label(f"Session: {svc.session_name}").classes("text-gray-400")
                ui.label(f"Queue: {len(svc.get_queue())} songs").classes("text-gray-400")
                ui.button(
                    "Resume Session",
        ...
        if not device_options:
            ui.label("No Spotify devices found. Open Spotify on a device first.").classes(
        ...
        def start_new():
            if not device_select.value:
                ...
                return
            svc.start_session(session_name.value, device_select.value)
            ui.navigate.to("/dj")

        ui.button("Start Session", on_click=start_new).props("color=positive").classes("w-full")
