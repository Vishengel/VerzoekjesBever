...
class DJPage:
    ...
    def _build_search_panel(self):
        with ui.column().classes("w-1/2 p-6 gap-4 border-r border-gray-700 h-full"):
            ...
            self._search_results = ui.column().classes("w-full gap-2 overflow-auto flex-grow")
            ...
            with ui.row().classes("gap-2"):
                ...
                def beaver_toggle():
                    ...
                    ui.button(
                        ...
                        on_click=lambda: (self.svc.set_beaver_enabled(not enabled), beaver_toggle.refresh()),
    ...
    def _build_requester_input(self):
        with ui.card().classes("w-full bg-gray-900"):
            ...
            self._requester_input.on(
                "focus",
                lambda: self._requester_input.set_autocomplete(self.svc.get_known_requesters()),
            )
    ...
    def _on_search_input(self, e):
        ...
        if not query or len(query) < 2:
            ...
            return
        self._search_timer = ui.timer(0.4, lambda q=query: self._do_search(q), once=True)
    ...
    def _add_song(self, item: QueueItem, top: bool):
        requester = self._requester_input.value.strip() if self._requester_input.value else ""
        if not requester:
    ...
    def _render_playback_controls(self):
        ...
        with ui.row().classes("w-full justify-center gap-4"):
            if state == PlaybackState.PLAYING:
                ui.button("⏸ Pause", on_click=lambda: (self.svc.pause(), self._refresh_all())).props("color=warning")
            elif state == PlaybackState.PAUSED:
                ui.button("▶ Resume", on_click=lambda: (self.svc.resume(), self._refresh_all())).props("color=positive")
            ui.button("⏭ Next", on_click=lambda: (self.svc.play_next(), self._refresh_all())).props("color=primary")
    ...
    def _render_queue(self):
        ...
        if current:
            ...
            state_label = "Paused" if self.svc.playback_state == PlaybackState.PAUSED else "Ready to play"
            ...
            with ui.row().classes("items-center gap-4"):
                ...
                with ui.column().classes("flex-grow gap-0"):
                    ...
                    with ui.row().classes("items-center gap-1"):
                        ui.label(f"Requested by {current.requester}" if current.requester else "No requester").classes(
                            "text-sm text-orange-400"
                        )
                        ui.button(
                            icon="edit",
                            on_click=lambda uri=current.track_uri, name=current.requester: self._open_edit_requester(
                                uri, name
                            ),
    ...
    def _render_queue_item(self, index: int, item: QueueItem):
        with ui.card().classes("w-full bg-gray-900"):
            with ui.row().classes("items-center gap-3"):
                ui.label(str(index + 1)).classes("text-gray-500 font-bold w-6 text-center")
                if item.album_art_url:
                ...
                with ui.column().classes("flex-grow gap-0"):
                    ...
                    if item.requester:
                        with ui.row().classes("items-center gap-1"):
                            ...
                            ui.button(
                                icon="edit",
                                on_click=lambda uri=item.track_uri, name=item.requester: self._open_edit_requester(
                                    uri, name
                                ),
                            ).props("flat round dense size=xs color=orange")
                ui.button(
                    icon="vertical_align_top",
                    on_click=lambda uri=item.track_uri: (self.svc.move_to_top(uri), self._queue_display.refresh()),
                ).props("flat round dense color=warning")
                ui.button(
                    icon="delete",
                    on_click=lambda uri=item.track_uri: (self.svc.remove_from_queue(uri), self._queue_display.refresh()),
